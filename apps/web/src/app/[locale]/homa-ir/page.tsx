...
import { homaIr } from "@medcalc/calculators";
...
export function generateMetadata(props: {
  ...
  return buildCalcMetadata("homa-ir", props.params);
}

export default async function HomaIrPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <div className="space-y-8">
      <CalcJsonLd id="homa-ir" locale={locale} />
      <div>
      ...
      <CalcContent id="homa-ir" locale={locale} />
      ...
      <details className="glass-panel p-4 text-sm text-slate-600 dark:text-slate-300">
        ...
        <ol className="mt-3 list-decimal space-y-2 pl-5">
          {homaIr.calculator.references.map((r) => (
            <li key={r.pmid}>
              {r.citation}{" "}
              <a
