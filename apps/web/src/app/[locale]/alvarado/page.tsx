...
import { AlvaradoForm } from "@/components/AlvaradoForm";
...
export function generateMetadata(props: {
  ...
  return buildCalcMetadata("alvarado", props.params);
}

export default async function AlvaradoPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <div className="space-y-8">
      <CalcJsonLd id="alvarado" locale={locale} />
      <div>
      ...
      <CalcContent id="alvarado" locale={locale} />
      ...
      <details className="glass-panel p-4 text-sm text-slate-600 dark:text-slate-300">
        ...
        <ol className="mt-3 list-decimal space-y-2 pl-5">
          {alvarado.calculator.references.map((r) => (
            <li key={r.pmid}>
              {r.citation}{" "}
              <a
