...
import { ScoradForm } from "@/components/ScoradForm";
...
export function generateMetadata(props: {
  ...
  return buildCalcMetadata("scorad", props.params);
}

export default async function ScoradPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <div className="space-y-8">
      <CalcJsonLd id="scorad" locale={locale} />
      <div>
      ...
      <CalcContent id="scorad" locale={locale} />
      ...
      <details className="glass-panel p-4 text-sm text-slate-600 dark:text-slate-300">
        ...
        <ol className="mt-3 list-decimal space-y-2 pl-5">
          {scorad.calculator.references.map((r) => (
            <li key={r.pmid}>
              {r.citation}{" "}
              <a
