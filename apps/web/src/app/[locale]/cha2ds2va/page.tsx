...
import { Cha2ds2vaForm } from "@/components/Cha2ds2vaForm";
...
export function generateMetadata(props: {
  ...
  return buildCalcMetadata("cha2ds2va", props.params);
}

export default async function Cha2ds2vaPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <div className="space-y-8">
      <CalcJsonLd id="cha2ds2va" locale={locale} />
      <div>
        <p className="text-xs font-medium text-slate-500 dark:text-slate-400">
          {t("cha2ds2va.title")}
        </p>
        <h1 className="mt-2 text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-50">
          {t("cha2ds2va.subtitle")}
        </h1>
      </div>

      <Cha2ds2vaForm />

      <CalcContent id="cha2ds2va" locale={locale} />
      ...
      <details className="glass-panel p-4 text-sm text-slate-600 dark:text-slate-300">
        ...
        <ol className="mt-3 list-decimal space-y-2 pl-5">
          {cha2ds2va.calculator.references.map((r) => (
            <li key={r.pmid}>
              {r.citation}{" "}
              <a
