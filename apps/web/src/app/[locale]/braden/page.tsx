...
import { BradenForm } from "@/components/BradenForm";
...
export function generateMetadata(props: {
  ...
  return buildCalcMetadata("braden", props.params);
}

export default async function BradenPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <div className="space-y-8">
      <CalcJsonLd id="braden" locale={locale} />
      <div>
      ...
      <CalcContent id="braden" locale={locale} />
      ...
      <details className="glass-panel p-4 text-sm text-slate-600 dark:text-slate-300">
        ...
        <ol className="mt-3 list-decimal space-y-2 pl-5">
          {braden.calculator.references.map((r) => (
            <li key={r.pmid}>
              {r.citation}{" "}
              <a
