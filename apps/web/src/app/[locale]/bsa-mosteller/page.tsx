...
import { BsaMostellerForm } from "@/components/BsaMostellerForm";
...
export function generateMetadata(props: {
  ...
  return buildCalcMetadata("bsa-mosteller", props.params);
}

export default async function BsaMostellerPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <div className="space-y-8">
      <CalcJsonLd id="bsa-mosteller" locale={locale} />
      <div>
        <p className="text-xs font-medium text-slate-500 dark:text-slate-400">
          {t("bsaMosteller.title")}
        </p>
        <h1 className="mt-2 text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-50">
          {t("bsaMosteller.subtitle")}
        </h1>
      </div>

      <BsaMostellerForm />

      <CalcContent id="bsaMosteller" locale={locale} />
      ...
      <details className="glass-panel p-4 text-sm text-slate-600 dark:text-slate-300">
        ...
        <ol className="mt-3 list-decimal space-y-2 pl-5">
          {bsaMosteller.calculator.references.map((r) => (
            <li key={r.pmid}>
              {r.citation}{" "}
              <a
