...
import { ApgarForm } from "@/components/ApgarForm";
...
export function generateMetadata(props: {
  ...
  return buildCalcMetadata("apgar", props.params);
}

export default async function ApgarPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <div className="space-y-8">
      <CalcJsonLd id="apgar" locale={locale} />
      <div>
      ...
      <CalcContent id="apgar" locale={locale} />
      ...
      <details className="glass-panel p-4 text-sm text-slate-600 dark:text-slate-300">
        ...
        <ol className="mt-3 list-decimal space-y-2 pl-5">
          {apgar.calculator.references.map((r) => (
            <li key={r.pmid}>
              {r.citation}{" "}
              <a
