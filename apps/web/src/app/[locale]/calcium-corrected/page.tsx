...
export default async function CalciumCorrectedPage({
  ...
  return (
    <div className="space-y-8">
      <CalcJsonLd id="calcium-corrected" locale={locale} />
      <div>
        <p className="text-xs font-medium text-slate-500 dark:text-slate-400">
          {t("calciumCorrected.title")}
        </p>
        <h1 className="mt-2 text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-50">
          {t("calciumCorrected.subtitle")}
        </h1>
      </div>

      <CalciumCorrectedForm />

      <CalcContent id="calcium-corrected" locale={locale} />
      ...
      <details className="glass-panel p-4 text-sm text-slate-600 dark:text-slate-300">
        ...
        <ol className="mt-3 list-decimal space-y-2 pl-5">
          {calciumCorrected.calculator.references.map((r) => (
            <li key={r.pmid}>
              {r.citation}{" "}
              <a
