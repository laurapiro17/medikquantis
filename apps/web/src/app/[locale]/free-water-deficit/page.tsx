...
import { freeWaterDeficit } from "@medcalc/calculators";
...
export function generateMetadata(props: {
  ...
  return buildCalcMetadata("free-water-deficit", props.params);
}

export default async function FreeWaterDeficitPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <div className="space-y-8">
      <CalcJsonLd id="free-water-deficit" locale={locale} />
      <div>
        <p className="text-xs font-medium text-slate-500 dark:text-slate-400">
          {t("freeWaterDeficit.title")}
        </p>
        <h1 className="mt-2 text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-50">
          {t("freeWaterDeficit.subtitle")}
        </h1>
      </div>
      ...
      <CalcContent id="free-water-deficit" locale={locale} />
      ...
      <details className="glass-panel p-4 text-sm text-slate-600 dark:text-slate-300">
        ...
        <ol className="mt-3 list-decimal space-y-2 pl-5">
          {freeWaterDeficit.calculator.references.map((r) => (
            <li key={r.pmid}>
              {r.citation}{" "}
              <a
