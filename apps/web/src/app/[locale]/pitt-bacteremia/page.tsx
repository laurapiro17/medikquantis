...
import { PittBacteremiaForm } from "@/components/PittBacteremiaForm";
...
export function generateMetadata(props: {
  ...
  return buildCalcMetadata("pitt-bacteremia", props.params);
}

export default async function PittBacteremiaPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <div className="space-y-8">
      <CalcJsonLd id="pitt-bacteremia" locale={locale} />
      <div>
        <p className="text-xs font-medium text-slate-500 dark:text-slate-400">
          {t("pittBacteremia.title")}
        </p>
        <h1 className="mt-2 text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-50">
          {t("pittBacteremia.subtitle")}
        </h1>
      </div>

      <PittBacteremiaForm />

      <CalcContent id="pitt-bacteremia" locale={locale} />
      ...
      <details className="glass-panel p-4 text-sm text-slate-600 dark:text-slate-300">
        ...
        <ol className="mt-3 list-decimal space-y-2 pl-5">
          {pittBacteremia.calculator.references.map((r) => (
            <li key={r.pmid}>
              {r.citation}{" "}
              <a
