...
import { NyhaForm } from "@/components/NyhaForm";
...
export function generateMetadata(props: {
  ...
  return buildCalcMetadata("nyha", props.params);
}

export default async function NyhaPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <div className="space-y-8">
      <CalcJsonLd id="nyha" locale={locale} />
      <div>
      ...
      <CalcContent id="nyha" locale={locale} />
      ...
      <details className="glass-panel p-4 text-sm text-slate-600 dark:text-slate-300">
        ...
        <ol className="mt-3 list-decimal space-y-2 pl-5">
          {nyha.calculator.references.map((r) => (
            <li key={r.pmid}>
              {r.citation}{" "}
              <a
