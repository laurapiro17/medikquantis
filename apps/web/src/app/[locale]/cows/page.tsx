...
import { CowsForm } from "@/components/CowsForm";
...
export function generateMetadata(props: {
  ...
  return buildCalcMetadata("cows", props.params);
}
...
export default async function CowsPage({
  ...
  return (
    <div className="space-y-8">
      <CalcJsonLd id="cows" locale={locale} />
      <div>
      ...
      <CalcContent id="cows" locale={locale} />
      ...
      <details className="glass-panel p-4 text-sm text-slate-600 dark:text-slate-300">
        ...
        <ol className="mt-3 list-decimal space-y-2 pl-5">
          {cows.calculator.references.map((r) => (
            <li key={r.pmid}>
              {r.citation}{" "}
              <a
