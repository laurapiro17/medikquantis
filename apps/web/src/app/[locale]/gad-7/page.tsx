...
export function generateMetadata(props: {
  ...
  return buildCalcMetadata("gad-7", props.params);
}

export default async function Gad7Page({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <div className="space-y-8">
      <CalcJsonLd id="gad-7" locale={locale} />
      <div>
      ...
      <CalcContent id="gad-7" locale={locale} />
      ...
      <details className="glass-panel p-4 text-sm text-slate-600 dark:text-slate-300">
        ...
        <ol className="mt-3 list-decimal space-y-2 pl-5">
          {gad7.calculator.references.map((r) => (
            <li key={r.pmid}>
              {r.citation}{" "}
              <a
