...
import { SadPersonsForm } from "@/components/SadPersonsForm";
...
export function generateMetadata(props: {
  ...
  return buildCalcMetadata("sad-persons", props.params);
}

export default async function SadPersonsPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <div className="space-y-8">
      <CalcJsonLd id="sad-persons" locale={locale} />
      <div>
        <p className="text-xs font-medium text-slate-500 dark:text-slate-400">
          {t("sadPersons.title")}
        </p>
        <h1 className="mt-2 text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-50">
          {t("sadPersons.subtitle")}
        </h1>
      </div>

      <SadPersonsForm />

      <CalcContent id="sad-persons" locale={locale} />
      ...
      <details className="glass-panel p-4 text-sm text-slate-600 dark:text-slate-300">
        ...
        <ol className="mt-3 list-decimal space-y-2 pl-5">
          {sadPersons.calculator.references.map((r) => (
            <li key={r.pmid}>
              {r.citation}{" "}
              <a
