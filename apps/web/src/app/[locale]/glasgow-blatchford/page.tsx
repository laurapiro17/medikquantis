...
export default async function GlasgowBlatchfordPage({
  ...
  return (
    <div className="space-y-8">
      <CalcJsonLd id="glasgow-blatchford" locale={locale} />
      <div>
        <p className="text-xs font-medium text-slate-500 dark:text-slate-400">
          {t("glasgowBlatchford.title")}
        </p>
        <h1 className="mt-2 text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-50">
          {t("glasgowBlatchford.subtitle")}
        </h1>
      </div>

      <GlasgowBlatchfordForm />

      <CalcContent id="glasgow-blatchford" locale={locale} />
      ...
      <details className="glass-panel p-4 text-sm text-slate-600 dark:text-slate-300">
        ...
        <ol className="mt-3 list-decimal space-y-2 pl-5">
          {glasgowBlatchford.calculator.references.map((r) => (
            <li key={r.pmid}>
              {r.citation}{" "}
              <a
