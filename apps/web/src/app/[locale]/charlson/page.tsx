...
import { CharlsonForm } from "@/components/CharlsonForm";
...
export function generateMetadata(props: {
  ...
  return buildCalcMetadata("charlson", props.params);
}

export default async function CharlsonPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <div className="space-y-8">
      <CalcJsonLd id="charlson" locale={locale} />
      <div>
      ...
      <CalcContent id="charlson" locale={locale} />
      ...
      <details className="glass-panel p-4 text-sm text-slate-600 dark:text-slate-300">
        ...
        <ol className="mt-3 list-decimal space-y-2 pl-5">
          {charlson.calculator.references.map((r) => (
            <li key={r.pmid}>
              {r.citation}{" "}
              <a
