...
export function generateMetadata(props: {
  ...
  return buildCalcMetadata("phq-9", props.params);
}

export default async function Phq9Page({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <div className="space-y-8">
      <CalcJsonLd id="phq-9" locale={locale} />
      <div>
      ...
      <CalcContent id="phq-9" locale={locale} />
      ...
      <details className="glass-panel p-4 text-sm text-slate-600 dark:text-slate-300">
        ...
        <ol className="mt-3 list-decimal space-y-2 pl-5">
          {phq9.calculator.references.map((r) => (
            <li key={r.pmid}>
              {r.citation}{" "}
              <a
