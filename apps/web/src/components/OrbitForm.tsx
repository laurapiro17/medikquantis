...
function ResultPanel(props: {
  ...
  const tierKey = `orbit.patient.result_${props.tier}` as const;
  return (
    <div className="animate-fade-in space-y-4 rounded-lg border border-slate-200 bg-white p-6">
      <p className="text-slate-700">{t("orbit.patient.intro")}</p>
      ...
      <div>
        <p className="font-medium text-slate-900">
          {t("orbit.patient.ask_doctor")}
        </p>
        <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-slate-700">
          {t.raw("orbit.patient.questions") instanceof Array
            ? (t.raw("orbit.patient.questions") as string[]).map((q, i) => (
                <li key={i}>{q}</li>
              ))
            : null}
        </ul>
